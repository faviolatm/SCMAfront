// src/pages/Evaluation/components/Analytics/LowScoresByRegion.jsx
import React from 'react';

const LowScoresByRegion = ({ data, loading, threshold }) => {
  if (loading) {
    return (
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-gray-800/50 border-2 border-gray-700 rounded-xl p-8 text-center">
        ...
        <p className="text-gray-400 text-sm">No Regions with low scores</p>
      </div>
    );
  }
  ...
  const getRegionIcon = (region) => {
    ...
    if (regionName.includes('AMERICAS') || regionName.includes('AMERICA')) {
      return '🌎';
    } else if (regionName.includes('EUROPE') || regionName.includes('EMEA')) {
      return '🌍';
    } else if (regionName.includes('ASIA') || regionName.includes('AP') || regionName.includes('PACIFIC')) {
      return '🌏';
    } else if (regionName.includes('GLOBAL')) {
      return '🌐';
    }
    return '📍';
  };
  ...
  return (
    <div className="bg-gray-800/50 border-2 border-green-500 rounded-xl overflow-hidden h-full flex flex-col">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-900/30 to-green-800/30 p-5 border-b border-green-500/50 flex-shrink-0">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Low Scores by Region</h3>
            <p className="text-green-300 text-xs">Threshold ≤ {threshold}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-hidden">
        <div className="max-h-[400px] overflow-y-auto custom-scrollbar p-5 space-y-4">
          {data.map((region, index) => {
            const percentage = (region.total_low_scores / maxScore) * 100;
            const barColor = getBarColor(region.avg_low_score);
            const textColor = getTextColor(region.avg_low_score);
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span className="text-gray-500 text-xs">
                      ({region.categories_affected} categories)
                    </span>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className={`text-sm font-bold ${textColor}`}>
                      Avg: {region.avg_low_score.toFixed(2)}
                    </span>
                    <span className="text-white font-bold text-sm">
                      {region.total_low_scores}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative h-8 bg-gray-700 rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    {percentage > 20 && (
                      <span className="text-white text-xs font-semibold flex items-center gap-1">
                        ...
                        {region.lowest_score === 0 ? 'CRITICAL' : `Low: ${region.lowest_score}`}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900/80 pointer-events-none">
                    <span className="text-white text-xs font-semibold">
                      {region.total_low_scores} low score{region.total_low_scores !== 1 ? 's' : ''} • 
                      Lowest: {region.lowest_score}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div className="bg-gray-900/50 px-5 py-3 border-t border-gray-700 flex-shrink-0">
        <p className="text-gray-400 text-xs flex items-center gap-2">
          ...
          Showing {data.length} Region{data.length !== 1 ? 's' : ''} with low scores
        </p>
      </div>
...
export default LowScoresByRegion;